...
export default function ModalRechazarAgent({ open, onClose, solicitud, onConfirm }) {
  ...
  // Soportar render condicional o prop `open`.
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content" style={{ width: 520 }}>
        ...
        <textarea value={motivo} onChange={e => setMotivo(e.target.value)} rows={5} style={{ width: '100%' }} />
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 8 }}>
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-danger" onClick={() => {
            if (!motivo.trim()) { toast.warn('Ingrese un motivo'); return; }
