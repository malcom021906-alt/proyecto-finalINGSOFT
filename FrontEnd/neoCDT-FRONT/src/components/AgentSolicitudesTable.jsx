...
export default function AgentSolicitudesTable(props) {
  ...
  if (!solicitudes || solicitudes.length === 0) {
    return <div style={{ padding: 12 }}>No hay solicitudes pendientes</div>;
  }
              ...
              <td>
                {(() => {
                  ...
                  if (raw.includes('aprob')) {
                    clase = 'Aprobada';
                    label = 'Aprobada';
                  } else if (raw.includes('rechaz')) {
                    clase = 'Rechazada';
                    label = 'Rechazada';
                  } else if (raw.includes('valid') || raw.includes('pend')) {
                    clase = 'Pendiente';
                    label = 'En validación';
                  }
              ...
              <td>
                <div style={{ display: 'flex', gap: 6 }}>
                  <button
                    className="btn"
                    disabled={
                      String(s.estado).toLowerCase() !== 'en validación' &&
                      String(s.estado).toLowerCase() !== 'en_validacion'
                    }
                    onClick={() => onApprove(s)}
                  >
                    Aprobar
                  ...
                  <button className="btn btn-ghost" onClick={() => onOpenHist(s)}>
                    Historial
                  </button>
                </div>
              </td>
